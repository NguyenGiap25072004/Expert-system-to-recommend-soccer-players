...
def create_gui():
    ...
    tk.Label(frame_image, text="Hệ chuyên gia gợi ý cầu thủ bóng đá", font=("Arial", 14), bg="#ffffff", fg="#000000").pack(pady=5)
    ...
    # Thêm văn bản vào bên dưới ảnh
        text_label1 = tk.Label(frame_image, text="1. Mục tiêu của hệ thống: Xây dựng một hệ thống chuyên gia có khả năng gợi ý cầu thủ bóng đá dựa trên các đặc điểm mà người dùng mong muốn.", 
                            font=("Arial", 10), bg="#ffffff", fg="#000000", wraplength=180, justify="left")
        text_label1.pack(side="top", pady=10)  # Đặt văn bản bên dưới ảnh
        text_label2 = tk.Label(frame_image, text="2. Lợi ích của hệ thống: Hỗ trợ người dùng tìm kiếm cầu thủ phù hợp với yêu cầu một cách nhanh chóng và hiệu quả. Cung cấp thông tin chi tiết về cầu thủ, bao gồm các thuộc tính, kỹ năng và chỉ số. Giúp người dùng ra quyết định trong việc lựa chọn cầu thủ, ví dụ như trong game quản lý bóng đá hoặc tuyển chọn cầu thủ thực tế.", 
                            font=("Arial", 10), bg="#ffffff", fg="#000000", wraplength=180, justify="left")
        text_label2.pack(side="top", pady=10)  # Đặt văn bản bên dưới ảnh
        text_label3 = tk.Label(frame_image, text="3. Phương pháp tiếp cận: Sử dụng phương pháp suy diễn tiến, dựa trên luật (rule-based reasoning) để gợi ý cầu thủ.", 
                            font=("Arial", 10), bg="#ffffff", fg="#000000", wraplength=180, justify="left")
        text_label3.pack(side="top", pady=10)  # Đặt văn bản bên dưới ảnh
...
def diagnose(selected_event_listbox, result_text):
    ...
    if matched_players:
        for idx, player in enumerate(matched_players, start=1):  # Đánh số thứ tự từ 1
            ...
            result_text.insert(tk.END, f"   Câu lạc bộ: {player['team']}\n")
            ...
            #result_text.insert(tk.END, f"   Giá trị của cầu thủ theo Linear Regression: {player['predicted_value_linear_regression']} Euro\n")
            result_text.insert(tk.END, f"   Giá trị thực của cầu thủ: {player['actual_value']} Euro\n")
            result_text.insert(tk.END, "-" * 90 + "\n")
